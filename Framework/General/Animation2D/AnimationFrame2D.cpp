...
void AnimationFrame2D::LoadFrameTexture()
{
	...
	//mp.texturePath = mFrameBasePath;
	//mp.textureName = mFramePath;
	//mp.tt = PNG;
	mp.in2D = true;
	...
	m_FrameTexture = new ZShadeSandboxMesh::QuadMesh(m_D3DSystem, mp, true);
	//m_FrameTexture->Initialize("Render", mFrameBasePath, mFramePath, width, height);
}
//-----------------------------------------------------------------------------------------------------------------