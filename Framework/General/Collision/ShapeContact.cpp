...
ZShadeSandboxMath::XMMath3 ShapeContact::NormalFromPoint(ZShadeSandboxMath::AABB box, ZShadeSandboxMath::XMMath3 point, float& distance)
{
	...
	ZShadeSandboxMath::XMMath3 center(box.vCenter);
	float fmin = FLT_MAX;
	...
	distance = abs(box.vExtent.x - abs(point.x));
	if (distance < fmin)
	{
		fmin = distance;
	...
	distance = abs(box.vExtent.y - abs(point.y));
	if (distance < fmin)
	{
		fmin = distance;
	...
	distance = abs(box.vExtent.z - abs(point.z));
	if (distance < fmin)
	{
		fmin = distance;
