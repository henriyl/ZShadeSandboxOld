...
//================================================================================================================================================================
//================================================================================================================================================================
void OceanHeightfield::CreateHeightfield(ZShadeSandboxMath::XMMath2* out_h0, float* out_omega)
{
	int i, j;
	ZShadeSandboxMath::XMMath2 K, Kn;
	ZShadeSandboxMath::XMMath2 wind_dir(wind_dir);
	
	wind_dir.Normalize();

	float a = wave_amplitude * 1e-7f;	// It is too small. We must scale it for editing.
	float v = wind_speed;
	float dir_depend = wind_dependency;

	int height_map_dim = displacement_map_dim;
	float patch_length = patch_size;
...
float OceanHeightfield::GaussRandom()
{
	...
	return sqrtf(-2 * logf(v1)) * cosf(2 * PI * v2);
}
//================================================================================================================================================================
float OceanHeightfield::Phillips(ZShadeSandboxMath::XMMath2 K, ZShadeSandboxMath::XMMath2 windDir, float v, float a, float dir_depend)
{
	//Largest Vertexsible wave from constant wind of velocity v
	...
	//Filter out waves moving opVertexite to wind
	if (Kcos < 0)
		phillips *= dir_depend;
...
void OceanHeightfield::Initialize()
{
	...
	heightfield_size = (displacement_map_dim + 4) * (displacement_map_dim + 1);
	h0_data = new ZShadeSandboxMath::XMMath2[heightfield_size * sizeof(ZShadeSandboxMath::XMMath2)];
	omega_data = new float[heightfield_size * sizeof(float)];

	heightfield_dim = displacement_map_dim;
