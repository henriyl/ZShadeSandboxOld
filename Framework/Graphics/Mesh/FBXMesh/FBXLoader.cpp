...
bool FBXLoader::Load(const char* filename)
{
	try
	{
		...
		QueryPerformanceCounter(&start);
		ProcessGeometry(m_pFbxScene->GetRootNode());
		QueryPerformanceCounter(&end);
	...
	for (uint32 i = 0; i < mTriangleCount; ++i)
	{
		...
		for (uint32 j = 0; j < 3; ++j)
		{
			...
			for (uint32 i = 0; i < ctlPoint->blendingInfo.size(); ++i)
			{
				...
				blendingInfo.mBlendingIndex = currCtrlPoint->blendingInfo[i].blendingIndex;
				blendingInfo.mBlendingWeight = currCtrlPoint->blendingInfo[i].blendingWeight;
...
void FBXLoader::LoadVertexTexture(FbxMesh* mesh, int inCtrlPointIndex, int inTextureIndex, int inTextureLayer, XMFLOAT2& outTexture)
{
	...
	FbxGeometryElement* vertexUV = mesh->GetElementUV(inTextureLayer);
...
void FBXLoader::LoadControlPoints(FbxNode* node)
{
	...
	for (uint32 i = 0; i < controlPointCount; ++i)
	{
		...
		m_pControlPoints[i] = currCtrlPoint;
	}
}
//===============================================================================================================================
...
//===============================================================================================================================
void FBXLoader::LoadSkeletonHierarchy(FbxNode* inNode, int myIndex, int parentIndex)
{
	...
	{
		LoadSkeletonHierarchy(inNode->GetChild(i), mSkeleton.mJoints.size(), myIndex);
	}
}
//===============================================================================================================================
void FBXLoader::LoadJointsAndAnimation(FbxNode* inNode)
{
	...
	for (uint32 deformerIndex = 0; deformerIndex < numOfDeformers; ++deformerIndex)
	{
		...
		for (uint32 clusterIndex = 0; clusterIndex < numOfClusters; ++clusterIndex)
		{
			...
			mSkeleton.mJoints[jointIndex].globalBindposeInverse = globalBindposeInverse;
			mSkeleton.mJoints[jointIndex].node = cluster->GetLink();
			...
			Keyframe** anim = &mSkeleton.joints[jointIndex].animation;
