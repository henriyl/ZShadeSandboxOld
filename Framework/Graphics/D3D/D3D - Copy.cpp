...
bool D3D::InitializeBackbuffers11()
{
	...
	/*
		Setup the NormalScene Post Process to hold the RT/MRT and depth buffers for rendering
		and create the different post processes.
		The NormalScene will contain the GBuffer for deferred shading.
		The PostProcessManager will handle switching back and forth between the different render targets
	*/
	...
	//m_deviceContext->OMSetRenderTargets(1, &m_renderTargetView, m_pDepthStencilBuffer->GetDSView());
...
//===============================================================================================================================
bool D3D::BeginScene(bool PostProcessEnable, float red, float green, float blue, float alpha)
{
	HRESULT hr;
	...
	else if (mOptions->m_d3dVersion == DIRECTX11)
	{
		...
		//{
		//	ID3D11RenderTargetView* rtv[1] = { m_pColorRT->RTView };
		//	ID3D11DepthStencilView* dsv = m_pDepthStencilBuffer->GetDSView();
		...
		//	m_deviceContext->ClearRenderTargetView(rtv[0], color);
			...
			//m_deviceContext->ClearDepthStencilView(m_depthStencilView, D3D11_CLEAR_DEPTH, 1.0f, 0);//D3D11_CLEAR_STENCIL

		//	m_deviceContext->ClearDepthStencilView(dsv, D3D11_CLEAR_DEPTH, 1.0f, 0);
		//}
		//else
		//{
			ID3D11RenderTargetView* rtv[1] = { m_renderTargetViewBackbuffer };
			ID3D11DepthStencilView* dsv = m_pDepthStencilBackBufferView;
		...
		//}
			...
			/*if (iStartCount < 2)
			{
				...
				SetOceanRenderState(true);

				m_pDummyTargetShader->Run();

				SetOceanRenderState(false);
...
void D3D::ClearBackbufferStencil(float red, float green, float blue, float alpha)
{
	...
	//Clear the back buffer
	m_deviceContext->ClearRenderTargetView(m_renderTargetViewBackbuffer, color);
	m_deviceContext->ClearDepthStencilView(m_pDepthStencilBackBufferView, D3D11_CLEAR_DEPTH|D3D11_CLEAR_STENCIL, 1.0f, 0);
}
//===============================================================================================================================
void D3D::EndScene()
{
