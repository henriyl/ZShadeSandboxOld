...
using ZShadeSandboxTerrain::TerrainPointCollider;
//===============================================================================================================================
//===============================================================================================================================
TerrainPointCollider::TerrainPointCollider()
...
//bool TerrainPointCollider::CameraColliding(float unitsPerMeter, float gravity, Camera* camera, vector<Triangle*> internalTriangles, XMFLOAT3& intersectionPoint)
{
	...
	mCollisionPacket.wPosition = ZShadeSandboxMath::XMMath3(camera->Position()).ToVector();
	mCollisionPacket.wVelocity = (camera->MoveStrafe() * camera->VRight()) + (camera->MoveWalk() * camera->VForward());
	intersectionPoint = ZShadeSandboxMath::XMMath3(CollisionSlide(node->triangleVertices, node->triangleIndices));

	return mCollisionPacket.collisionFound;
}
//===============================================================================================================================
XMVECTOR TerrainPointCollider::CollisionSlide(vector<XMFLOAT3> internalVertices, vector<UINT> internalIndices)
{
	...
	XMVECTOR finalPosition = CollisionWithWorld(internalVertices, internalIndices);
	...
	finalPosition = CollisionWithWorld(internalVertices, internalIndices);
...
//===============================================================================================================================
XMVECTOR TerrainPointCollider::CollisionWithWorld(vector<XMFLOAT3> internalVertices, vector<UINT> internalIndices)
{
	...
	for (int triID = 0; triID < internalIndices.size() / 3; triID++)
	{
		vertex0 = ZShadeSandboxMath::XMMath3(internalVertices[internalIndices[3 * triID + 0]]).ToVector();
		vertex1 = ZShadeSandboxMath::XMMath3(internalVertices[internalIndices[3 * triID + 1]]).ToVector();
		vertex2 = ZShadeSandboxMath::XMMath3(internalVertices[internalIndices[3 * triID + 2]]).ToVector();
		...
		SphereCollidingWithTriangle(vertex0, vertex1, vertex2, triNormal);
	}
	...
	if (mCollisionPacket.nearestDistance >= veryCloseDistance)
	{
	...
	return CollisionWithWorld(internalVertices, internalIndices);
}
//===============================================================================================================================
bool TerrainPointCollider::SphereCollidingWithTriangle(XMVECTOR& vertex0, XMVECTOR& vertex1, XMVECTOR& vertex2, XMVECTOR& normal)
{
...
//===============================================================================================================================
bool TerrainPointCollider::CheckPointInTriangle(const XMVECTOR& point, XMVECTOR& vertex0, XMVECTOR& vertex1, XMVECTOR& vertex2)
{
...
//===============================================================================================================================
bool TerrainPointCollider::GetLowestRoot(float a, float b, float c, float maxR, float* root)
{
